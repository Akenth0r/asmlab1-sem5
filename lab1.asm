...
bitString:
		...
		cycle:
			...
			bts word[fs:bx], si ; Иначе устанавливаем бит с индексом si
			zero:
			inc si ; Переходим на следующий бит
			cmp si, 15 ; Если si < 15
			jne skip ; Ничего не делаем
			xor si, si ; Иначе обнуляем индекс
			add bx, 2 ; И переходим на следующее слово строки-результата
			skip:
